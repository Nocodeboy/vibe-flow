...
const ProjectCard: React.FC<ProjectCardProps> = ({ project, index, range, targetScale, progress, onProjectClick, isMobile }) => {
    ...
    // Smooth 3D Tilt Physics - only on desktop
    ...
    // Dynamic rotation based on physics - disabled on mobile
    const rotateX = isMobile ? 0 : mouseY;
    const rotateY = isMobile ? 0 : mouseX;

    // Scroll Parallax for Image - disabled on mobile
    const imageY = useTransform(progress, range, isMobile ? ['0%', '0%'] : ['0%', '15%']);

    return (
        <div ref={containerRef} className={`${isMobile ? 'min-h-[70vh]' : 'h-screen'} flex items-center justify-center ${isMobile ? 'relative' : 'sticky top-0'} ${isMobile ? '' : 'perspective-1000'}`}>
            <motion.div
                style={isMobile ? {} : {
                    scale,
            ...
            >
                ...
                >
                    {/* Dynamic Colored Shadow / Glow - Hidden on mobile */}
                    {!isMobile && (
                        <motion.div
                    ...
                    {/* Holographic Glare - Hidden on mobile */}
                    {!isMobile && (
                        <motion.div
                    ...
                    <motion.div
                        ...
                        style={isMobile ? {} : { transform: "translateZ(-20px) scale(1.1)" }}
                    >
                    ...
                    {/* Noise Texture Overlay - Hidden on mobile */}
                    {!isMobile && (
                    ...
                    {/* Content */}
                    <div className="relative z-20 h-full p-8 md:p-14 flex flex-col justify-between" style={isMobile ? {} : { transform: "translateZ(40px)" }}>

                        {/* Header */}
                        <div className="flex justify-between items-start">
                            <div className="flex gap-3">
                                <Badge variant="glass" size="sm" className={`${isMobile ? 'backdrop-blur-sm' : 'backdrop-blur-xl'} bg-white/5 border-white/10`}>
                                    {project.category}
                                </Badge>
                        ...
                        <div className="max-w-2xl overflow-hidden">
                            ...
                            {isMobile ? (
                                <div className="flex items-center gap-6">
                                    <Button
                                        variant="primary"
                                        className="rounded-full px-6 py-3 bg-white text-black whitespace-nowrap text-sm"
...
const StackingProjects: React.FC<StackingProjectsProps> = ({ onProjectClick }) => {
    ...
    return (
        <section className="bg-[#030303] relative pt-20" id="projects">
            ...
            {/* Stacking Cards Container - Simplified layout on mobile */}
            <div ref={containerRef} className={`relative ${isMobile ? 'pb-10 flex flex-col gap-8' : 'pb-[20vh]'}`}>
                {projects.map((project, index) => {
                    // Calculate scale for stacking effect - disabled on mobile
